  ...
  if (!message.member.hasPermission("KICK_MEMBERS")) return message.reply("شما دسترسی اخراح کردن ندارید <a:738016054651125770:740118761192947755> ");
    ...
    if (command === 'server') {
        ...
        const server = new Discord.MessageEmbed()
            ...
            .addField("🌐Server Members:", `${memberCount}`, true)
            .addField("<a:star:753649951090802849> Server Roles", `${roles.cache.size}`, true)
            .addField("😉 Server Emojis", `${emojis.cache.size}`, true)
            .addField("📣 Server Channels", `${channels.cache.size}`, true)
            .addField("🎙 Server Voices", `${voiceStates.cache.size}`, true)
            .addField("<a:boost:753169213744611418>Server Boosts", `${premiumSubscriptionCount}`, true)
            .addField("<a:nitro:777406909354606602>Server Boost Level", `${premiumTier}`, true)
            .addField("👑 Server Owner", `<@${ownerID}>`, true)
            .setFooter(`Coded By ${owner.tag}`, `${owner.displayAvatarURL({size: 2048, dynamic:true})}`)
            .setImage(nobanner)
